...
exports.saveexpense = function (req, res, next) {
  ...
  const type = req.body.expaccount;
...
exports.expensetotal = function (req, res, next) {
  ...
  if (rptype == 'opt1') {
    let oDt = new Date();
    let month = oDt.getUTCMonth() + 1; //months 1-12
...
exports.expensereport = function (req, res, next) {
  ...
  if (!uid || !rptype) {
    ...
    res.status(422).send({ error: 'From or To date missing' });
  }
  else if (fromdt > todt) {
    res.status(422).send({ error: 'From date cannot be greater than To date' });
  }
  else {
    if (rptype == "opt1") {
      ...
      query = { userid: uid };
      Expense.count(querry, function (err, count) {
        if (count > offset) {
          offset = 0;
        }
      });
